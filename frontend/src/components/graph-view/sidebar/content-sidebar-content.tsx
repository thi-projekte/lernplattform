    ...
    return (
        <>
            <Group justify="space-between" align="flex-start">
                <div>
                    <Title order={3} style={{ lineHeight: 1.2 }}>{selectedElement.title}</Title>
                    <Badge color="teal" mt="xs">{t(`topic.contentElementType.${selectedElement.type}`)}</Badge>
                </div>
            ...
            <Modal opened={opened} onClose={close} size="xl" title={<Title order={3}>{selectedElement.title}</Title>}>
                <ContentElementDisplay contentElement={selectedElement} />
            </Modal>
        </>
    )
