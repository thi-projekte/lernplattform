...
public final class TopicGraphResource {
  ...
  @Operation(
          ...
          description = "Returns the most popular topics in the graph along with their direct neighbors. " +
                  "Without filters returns the top 10 globally, or top 100 when personalized for the current user."
  )
  @Parameter(name = "categories", description = "Optional list of category IDs to filter topics by. If empty, all categories are considered.")
  @Parameter(name = "personal", description = "If true, results are personalized for the current user and the limit is increased to 100.")
  @APIResponse(responseCode = "200", description = "List of popular topics with their direct neighbors")
  ...
  @Operation(
          ...
          description = "Returns all direct neighbors of the given topic in the graph. " +
                  "When personalized, only returns neighbors owned by the current user."
  )
  @Parameter(name = "topicId", description = "The unique ID of the topic whose neighbors should be retrieved.")
  @Parameter(name = "personal", description = "If true, only neighbors owned by the current user are returned.")
