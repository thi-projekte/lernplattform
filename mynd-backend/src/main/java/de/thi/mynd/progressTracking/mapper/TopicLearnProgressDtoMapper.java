...
@ApplicationScoped
public final class TopicLearnProgressDtoMapper extends AbstractMappingProcessor<LearnProgressTopic, TopicLearnProgressDto> {
    @Override
    public TopicLearnProgressDto mapAndEnrich(LearnProgressTopic entity) {
        ...
        List<UUID> completedIds = elements.stream()
                .filter(e -> e.completed)
                .map(e -> e.id.contentElementId)
                .toList();

        boolean completed = entity.status == LearnProgressStatus.COMPLETED_MANUALLY || entity.status == LearnProgressStatus.ALL_CONTENT_ELEMENTS_COMPLETED;

        double percentageCompleted = (double) completedIds.size() / (double) entity.contentElementsToComplete;
