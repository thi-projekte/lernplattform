...
class TopicLearnProgressServiceImplTest {
    ...
    @Inject
    TopicLearnProgressService topicLearnProgressService;
    ...
    void getLearnProgressForTopic_returnsDto_whenTopicStarted() {
        ...
        when(learnProgressTopicRepository.findOneByTopicIdAndCreatorIdContentElementsFetched(topicId, CREATOR_ID))
    ...
    void getLearnProgressForTopic_throws_whenTopicNotStarted() {
        ...
        assertThrows(TopicLearnProgressNotStartedException.class,
    ...
    void startLearningTopicAsCurrentUser_persistsTopic_whenNotYetStarted() {
        ...
        verify(learnProgressTopicRepository).persistAndFlush(argThat(topic ->
    ...
    void startLearningTopicAsCurrentUser_throws_whenAlreadyStarted() {
        ...
        assertThrows(TopicLearnProgressAlreadyStartedException.class,
                () -> topicLearnProgressService.startLearningTopicAsCurrentUser(topicId));
    ...
    void startLearningTopicAsCurrentUser_setsContentElementCount_fromService() {
        ...
        verify(learnProgressTopicRepository).persistAndFlush(argThat(topic ->
                topic.contentElementsToComplete == 0L
        ));
    }
    ...
    void manuallyCompleteTopicAsCurrentUser_setsStatus_whenTopicStarted() {
        ...
        when(learnProgressTopicRepository.findByIdOptional(any())).thenReturn(Optional.of(progressTopic));

        topicLearnProgressService.manuallyCompleteTopicAsCurrentUser(topicId);

        verify(learnProgressTopicRepository).persistAndFlush(argThat(topic ->
                topic.status == LearnProgressStatus.COMPLETED_MANUALLY
        ));
    }
    ...
    void manuallyCompleteTopicAsCurrentUser_canCompleteAlreadyAutoCompleted() {
        UUID topicId = UUID.randomUUID();
        LearnProgressTopic progressTopic = buildProgressTopic(
                topicId, LearnProgressStatus.ALL_CONTENT_ELEMENTS_COMPLETED, 2);
    ...
    void completeContentElement_throws_whenAlreadyCompleted() {
        ...
        when(learnProgressContentElementRepository.findByContentElementIdAndCreatorId(contentElementId, CREATOR_ID))
                .thenReturn(Optional.of(existing));

        assertThrows(ContentElementLearnProgressAlreadyCompletedException.class,
    ...
    void completeContentElement_setsStatusToAllCompleted_whenLastElementCompleted() {
        ...
        verify(learnProgressTopicRepository).persistAndFlush(argThat(topic ->
                topic.status == LearnProgressStatus.ALL_CONTENT_ELEMENTS_COMPLETED
        ));
    }
    ...
    void completeContentElement_assignsCorrectCreatorId() {
        ...
        verify(learnProgressTopicRepository).persistAndFlush(argThat(topic ->
                topic.contentElements.get(0).id.creatorId.equals(CREATOR_ID)
        ));
    }
    ...
    private LearnProgressTopic buildProgressTopic(UUID topicId, LearnProgressStatus status, long elementsToComplete) {
