...
class StreakServiceImplTest {
  ...
  @InjectMock StreakRepository streakRepository;

  @InjectMock StreakPreferenceRepository streakPreferenceRepository;
  ...
  class GetLatestStreaks {
    ...
    void endsExpiredStreaks_andReturnsMappedDtos() {
      ...
      when(mappingRegistry.mapList(anyList(), eq(StreakDto.class)))
          .thenReturn(List.of(StreakDto.builder().build()));
  ...
  class ContinueOrStartStreaks {
    ...
    void inactiveStreakIsNotContinued_andNewOneIsCreated() {
      ...
      when(streakRepository.findNotEndedByCreatorId(CREATOR_ID)).thenReturn(List.of(inactiveDaily));
