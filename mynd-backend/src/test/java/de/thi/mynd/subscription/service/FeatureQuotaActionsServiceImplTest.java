...
class FeatureQuotaActionsServiceImplTest {
    ...
    @Inject
    FeatureQuotaActionsServiceImpl service;
    ...
    @Test
    void learnContentElement_WithinFreeLimit_ShouldIncrementAndPersist() throws FeatureQuotaHitException {
        mockQuota.count = 2; // Increments to 3 (<= 3 limit)

        // Mock repository behavior instead of service method spy
        when(featureQuotaRepository.findByCreatorAndFeatureAndDate(eq(USER_ID), eq(Feature.LearnContentElementOrTopic), any(LocalDate.class)))
    ...
    void learnContentElement_ExceedsFreeLimitWithoutPremium_ShouldThrowException() {
        ...
        FeatureQuotaHitException exception = assertThrows(FeatureQuotaHitException.class, () -> {
            service.learnContentElement(USER_ID);
        });

        assertEquals("You cannot learn more content elements today", exception.getMessage());
        // Should only be persisted once if fallback happened, but service execution should halt before saving bad state
        verify(featureQuotaRepository, never()).persistAndFlush(mockQuota);
    }

    @Test
    void learnContentElement_FallbackToNewQuota_CreatesSuccessfully() throws FeatureQuotaHitException {
        // Mock database returning empty (no existing quota found for today or ever)
        when(featureQuotaRepository.findByCreatorAndFeatureAndDate(any(), any(), any())).thenReturn(Optional.empty());
    ...
    @Test
    void completeTopic_CountGreaterThanZero_ShouldDecrementAndPersist() throws FeatureQuotaHitException {
        mockQuota.count = 2;
