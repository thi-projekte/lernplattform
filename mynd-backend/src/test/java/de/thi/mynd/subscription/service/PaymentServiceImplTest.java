...
class PaymentServiceImplTest {
    ...
    @Test
    void createInitialSubscriptionSession_withFreeSubscriptionAndExistingCustomer_returnsSessionDto() {
        ...
        when(stripeService.createCheckoutSessionForSubscriptionPrice(price, STRIPE_CUSTOMER_ID)).thenReturn(session);
        when(mappingRegistry.map(session, StripeSessionDto.class)).thenReturn(expectedDto);

        StripeSessionDto result = paymentService.createInitialSubscriptionSession(SubscriptionStatus.PRO);
    ...
    @Test
    void createInitialSubscriptionSession_withFreeSubscriptionAndNoCustomer_createsCustomerThenReturnsSessionDto() {
        ...
        when(subscriptionService.getSubscriptionForCurrentUser()).thenReturn(subscriptionWithoutCustomer);
    ...
    @Test
    void createInitialSubscriptionSession_withNonFreeSubscription_throwsCannotUpgradeSubscriptionException() {
    ...
    void createInitialSubscriptionSession_usesCurrentUserIdentityForCustomerLookup() {
        ...
        when(stripeService.createCheckoutSessionForSubscriptionPrice(any(), any())).thenReturn(mock(Session.class));
        when(mappingRegistry.map(any(), eq(StripeSessionDto.class))).thenReturn(StripeSessionDto.builder().build());
    ...
    @Test
    void createInitialSubscriptionSession_withDifferentSubscriptionStatus_passesCorrectStatusToStripe() {
        Subscription subscription = freeSubscriptionWithCustomer();
        Price price = mock(Price.class);
        Session session = mock(Session.class);
        ...
        when(stripeService.createCheckoutSessionForSubscriptionPrice(price, STRIPE_CUSTOMER_ID)).thenReturn(session);
        when(mappingRegistry.map(session, StripeSessionDto.class)).thenReturn(StripeSessionDto.builder().build());
