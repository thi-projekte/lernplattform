...
class S3ObjectStorageImplTest {
    ...
    @Inject
    S3ObjectStorageImpl s3ObjectStorage;
    ...
    void testTryDeleteObject() {
        ...
        CompletableFuture<DeleteObjectResponse> future = CompletableFuture.completedFuture(DeleteObjectResponse.builder().build());
        ...
        verify(s3Client).deleteObject(argThat((DeleteObjectRequest req) ->
                req.bucket().equals(BUCKET_NAME) && req.key().equals(objectKey)
        ));
    }
}