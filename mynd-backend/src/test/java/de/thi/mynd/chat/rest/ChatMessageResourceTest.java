...
class ChatMessageResourceTest {
    ...
    void testGetChatMessages_Success() {
        ...
        PaginationDto<ChatMessageDto> mockPaginationResult = PaginationDto.<ChatMessageDto>builder()
                .results(List.of(msg))
                .totalPages(2)
                .build();

        // Stub service response
        when(chatMessageService.getMessages(topicId, page, pageSize))
                .thenReturn(mockPaginationResult);
        ...
        // Verify the JAX-RS path/query parameters were unmarshalled and sent to the service layer properly
