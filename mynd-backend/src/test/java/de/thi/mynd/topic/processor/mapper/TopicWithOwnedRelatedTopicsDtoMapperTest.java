...
@QuarkusTest
class TopicWithOwnedRelatedTopicsDtoMapperTest {

  @Inject TopicWithOwnedRelatedTopicsDtoMapper mapper;
  ...
  @Test
  void mapAndEnrich_happyPath_mapsAllFieldsIncludingRelatedTopics() {
    Topic topic = topic();
    ...
    List<ListTopicDto> relatedTopics =
        List.of(ListTopicDto.builder().id(UUID.randomUUID()).title("Related Topic").build());
    ...
    when(topicService.getOwnedRelatedTopicsForTopic(topic.id)).thenReturn(relatedTopics);

    TopicWithOwnedRelatedTopicsDto dto = mapper.mapAndEnrich(topic);
    ...
    assertEquals(relatedTopics, dto.relatedTopics);
    assertNull(dto.learnProgress);
    verify(topicService).getOwnedRelatedTopicsForTopic(topic.id);
  }

  @Test
  void mapAndEnrich_noRelatedTopics_relatedTopicsIsEmpty() {
    Topic topic = topic();
    ...
    when(topicService.getOwnedRelatedTopicsForTopic(topic.id)).thenReturn(List.of());

    TopicWithOwnedRelatedTopicsDto dto = mapper.mapAndEnrich(topic);

    assertTrue(dto.relatedTopics.isEmpty());
  }
  ...
  void mapAndEnrichWithAdditionalData_progressMapContainsTopic_setsLearnProgress() {
    ...
    TopicWithOwnedRelatedTopicsDto dto = mapper.mapAndEnrich(topic, (Object) progressMap);
  ...
  void getEntityType_returnsTopic() {
    assertEquals(Topic.class, mapper.getEntityType());
  }

  @Test
  void getDtoType_returnsTopicWithOwnedRelatedTopicsDto() {
    assertEquals(TopicWithOwnedRelatedTopicsDto.class, mapper.getDtoType());
  }
}
