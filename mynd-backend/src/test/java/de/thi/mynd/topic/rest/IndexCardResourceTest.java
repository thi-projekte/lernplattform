...
class IndexCardResourceTest {
    ...
    @Test
    @TestSecurity(user = "bob", roles = {"authorizedUser", "builder"})
    void testCreateContentElement_Success() {
        ...
        given()
                ...
                .post("/index-cards")
                .then()
                .statusCode(200); // Note: Since your resource returns the DTO directly instead of Response.created(), JAX-RS defaults to a 200 OK
    ...
    void testDeleteContentElement_Returns404_WhenServiceThrowsNotFound() {
        ...
        doThrow(new EntityInstanceNotFoundException("Not Found"))
                .when(indexCardService).deleteIndexCard(cardId);
        ...
        given()
                ...
                .then()
                .statusCode(404);
        // Note: This relies on you having an ExceptionMapper that maps EntityInstanceNotFoundException -> 404 Status.
    }
}