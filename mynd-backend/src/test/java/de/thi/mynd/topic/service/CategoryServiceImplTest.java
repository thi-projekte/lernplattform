...
class CategoryServiceImplTest {
    ...
    void testSearch_WithQuery() {
        // Arrange
        String query = "Java";
        when(categoryRepository.findByTitleWithLimit(eq(query), eq(5))).thenReturn(List.of(new Category()));
    ...
    void testFindByAssociatedEntities() {
        ...
        when(categoryRepository.findByIdsTypeSafe(anyList())).thenReturn(List.of(new Category(), new Category()));
        ...
        verify(categoryRepository).findByIdsTypeSafe(argThat(list ->
                list.contains(id1) && list.contains(id2) && list.size() == 2
        ));
    }
}