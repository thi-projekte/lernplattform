...
/**
 * Assumptions:
 * - ImportContext has: isBackendMode(), getCategoryMapping(), getTopicMapping(),
 *   withCategoryMapping(Map), withTopicMapping(Map)
 * - ImportException extends RuntimeException
 * - Category fields: creatorId, title, color (public or package-private like Topic)
 * - Topic fields: title, teaser, creatorId, estimatedLearningDuration, categories (public)
 * - TopicAssociation fields: creatorId, owningTopic, foreignTopic (public)
 * - ContentElement has a `topic` field
 ...
 * - FullImportDto has getTopics(), getAssociations() — no categories in full import
 *   (matches importFull which skips doImportCategories)
 */
@QuarkusTest
class ImportServiceImplTest {
    ...
    class ImportCategories {
        ...
        void happyPath_backendMode_persistsAndReturnsMapping() {
            ...
            verify(categoryRepository).persist(argThat((Category c) ->
        ...
        void happyPath_userMode_usesIdentityAsCreator() {
            ...
            verify(categoryRepository).persist(argThat((Category c) ->
                    c.creatorId.equals(USER_PRINCIPAL)
            ));
            assertTrue(result.getCategoryMapping().containsKey("cat-1"));
        }
        ...
        void multipleCategories_allPersistedAndMapped() {
            ImportContext ctx = new ImportContext(true);
            List<ImportCategoryDto> dtos = List.of(
        ...
        void duplicateIdentifier_lastOneWinsInMapping() {
            ...
            List<ImportCategoryDto> dtos = List.of(
                    categoryDto("cat-1", "First", "#111"),
                    categoryDto("cat-1", "Second", "#222")
            );
    ...
    class ImportTopics {
        ...
        void happyPath_backendMode_persistsTopicAndContentElements() {
            ...
            ImportTopicDto dto = topicDto("topic-1", "Java Basics", "Learn Java",
                    List.of("cat-ref"), List.of(rtfElement("Hello")));

            ImportContext result = importService.importTopics(List.of(dto), ctx);

            verify(topicRepository).persist(argThat((Topic t) ->
        ...
        void categoryResolution_fromContextMapping() {
            ...
            ImportContext ctx = new ImportContext(true)
                    .withCategoryMapping(Map.of("cat-key", cat));
            ImportTopicDto dto = topicDto("t-1", "T", "T", List.of("cat-key"), List.of());

            importService.importTopics(List.of(dto), ctx);

            verify(topicRepository).persist(argThat((Topic t) ->
                    t.categories.contains(cat)
            ));
        ...
        void categoryResolution_fromDatabase_whenNotInMapping() {
            ...
            ImportContext ctx = new ImportContext(true);
            ImportTopicDto dto = topicDto("t-1", "T", "T",
                    List.of(EXISTING_CATEGORY_ID.toString()), List.of());
        ...
        void categoryResolution_unknownUUID_throwsImportException() {
            ...
            ImportContext ctx = new ImportContext(true);
            ImportTopicDto dto = topicDto("t-1", "T", "T",
                    List.of(unknownId.toString()), List.of());
        ...
        void categoryResolution_invalidKey_notUUIDAndNotInMapping_throwsImportException() {
            ImportContext ctx = new ImportContext(true);
            ImportTopicDto dto = topicDto("t-1", "T", "T",
                    List.of("this-is-not-a-uuid"), List.of());
        ...
        void multipleContentElements_allPersistedWithCorrectTopic() {
            ImportContext ctx = new ImportContext(true);
            ImportTopicDto dto = topicDto("t-1", "T", "T", List.of(),
                    List.of(rtfElement("A"), rtfElement("B"), rtfElement("C")));

            importService.importTopics(List.of(dto), ctx);

            verify(contentElementRepository, times(3)).persist(argThat((ContentElement ce) ->
                    ce.topic != null
            ));
        }
    ...
    class ImportAssociations {
        ...
        void happyPath_createsAssociation() {
            ...
            ImportContext ctx = new ImportContext(true)
                    .withTopicMapping(Map.of("owning", owning, "foreign", foreign));
            ...
            verify(topicAssociationRepository).persist(argThat((TopicAssociation a) ->
        ...
        void topicResolution_fromDatabase_whenNotInMapping() {
            ...
            when(topicRepository.findByIdOptional(EXISTING_TOPIC_ID))
                    .thenReturn(Optional.of(dbTopic));

            Topic foreign = persistedTopic("foreign");
            ImportContext ctx = new ImportContext(true)
                    .withTopicMapping(Map.of("foreign", foreign));
            when(topicAssociationRepository.associationExists(any(), any())).thenReturn(false);
        ...
        void topicResolution_unknownUUID_throwsImportException() {
            ...
            assertThrows(ImportException.class, () -> importService.importAssociations(
                    Map.of(unknownId.toString(), List.of()), ctx));
        }
        ...
        void multipleForignTopics_allProcessed() {
            ...
            importService.importAssociations(
                    Map.of("owning", List.of("f1", "f2", "f3")), ctx);
        ...
        void mixedExistingAndNew_onlyNewOnesArePersisted() {
            ...
            importService.importAssociations(
                    Map.of("owning", List.of("existing", "new")), ctx);

            verify(topicAssociationRepository, times(1)).persist(argThat((TopicAssociation a) ->
                    a.foreignTopic == newTopic
            ));
        }
    }
    ...
    class ImportFull {
        ...
        void topicMappingPassedToAssociations() {
            ...
            FullImportDto dto = new FullImportDto();
            dto.setTopics(List.of(topicDto("t-1", "Owner", "T", List.of(), List.of()),
    ...
    private ImportTopicDto topicDto(String identifier, String title, String teaser,
